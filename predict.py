...
parser.add_argument("-model"
                    , type=str
                    , nargs='+'
                    , help='model of ag file path')
parser.add_argument("-name"
                    , type=str
                    , nargs='?'
                    , default = 'now_model_ans'
                    , help='model of ag file path')
...
output_path = r"./" + args.name + r"/"
os.makedirs(output_path, exist_ok=True)

predictor = ImagePredictor.load(filename)

_dir = 'testing_img_order.txt'
input_txt = open(_dir, 'r')
f_txt = open( output_path + 'answer.txt', 'w' )
...
for line in input_txt:
    ...
    print(str(cnt) + 'th image:' + cl[0] + ' completed')
    _dir = './test/'+cl[0]
    prr = predictor.predict(_dir)
    f_txt.write("%s %s\n" % (str(cl[0]), str(labels[int(prr.values[0])])))
