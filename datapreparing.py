...
# Directory
for i in range(200):
    now_path =  r"./train/" + str(labels[i]) + r"/"
    os.makedirs(now_path, exist_ok=True)

new_test_path =  r"./test/"
os.makedirs(new_test_path, exist_ok=True)
image_path = r"./2021VRDL_HW1_datasets/training_images/"  # image path
test_image_path = r"./2021VRDL_HW1_datasets/testing_images/"

filenames = os.listdir(test_image_path)
image_file = []

for filename in filenames:
    image_file.append(str(filename))

import shutil
from PIL import Image

_dir = '2021VRDL_HW1_datasets/training_labels.txt'
input_txt = open(_dir, 'r')
...
for line in input_txt:
    ...
    testline = line[:-1]
    cl = [str(x) for x in testline.split()]
    print(cl[0] + ' is in file of ' + cl[1])
    orig_image = image_path + cl[0]
    tar_image = r"./train/" + str(cl[1]) + r"/"+ str(cl[0])
...
for image in image_file:
    print(image)
    org_img = test_image_path + image
    tar_img = new_test_path + image
